...
import { Cliente, InvoceReport,CotizacionReport } from '../models/InvoceReport';
...
@Component({
  ...
  styleUrls: ['./editar-cotizaciones.component.css']
})
export class EditarCotizacionesComponent implements OnInit {
  ...
  facturaSeleccionada:CotizacionReport;
  ...
  eliminar(id:number){
    ...
    let retencionesArray = this.clienteSeleccionado.retencionCliente.map(ret => ret.retencion.porcentaje)

    console.log(retencionesArray)
    ...
    new MetodosShared().AlertQuestion(
      ...
      if (result.isConfirmed) {
        ...
        this.invoceService.EliminarDetalle(id,retencionesArray).subscribe((resp:any) => {
  ...
  agregar(id:number,detalle:DetalleFactura){
    new MetodosShared().AlertQuestion(
      ...
      if (result.isConfirmed) {
        ...
        this.invoceService.AgregarDetalleCotizacion(id,detalle,this.clienteSeleccionado.retencionCliente).subscribe((resp:any) => {
  ...
  guardarFactura(){
    this.mostrarOpcionesDomicilio().then(() => {
      ...
      new MetodosShared().AlertQuestion(
        '¿ ESTA SEGURO DE ACTUALIZAR LA COTIZACION ?'
      ).then((result) => {
        if (result.isConfirmed) {
          ...
          this.invoceService.updateFactura(this.formfacturacion).subscribe((resp:any) => {
            ...
            new MetodosShared().AlertOK('COTIZACION ACTUALIZADA CON EXITO')
            this.resetformFactura();
            this.invoceService.cargarCotizaciones();
            this.route.navigateByUrl("facturacion/cotizacion");
  ...
  cargarFactura(){
    if(this.rutaActiva.snapshot.params.id){
      ...
      Swal.showLoading();
      this.invoceService.obtenerCotizacion(id).subscribe((resp:CotizacionReport) => {
