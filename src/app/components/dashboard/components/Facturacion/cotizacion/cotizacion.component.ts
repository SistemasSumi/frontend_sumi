...
export class CotizacionComponent implements OnInit {
  ...
  formCotizacion = this.formBuilder.group({
    id: ['', {}],
    numeracion: [
      '',
      {
  ...
  constructor(
    ...
    private invoceServiceCotizacion: CotizacionService,
  ...
  obtenerNumeraciones() {
    this.invoceServiceCotizacion
      .cargarNumeracionCotizaciones()
      .subscribe((resp: ModelNumeraciones[]) => {
  ...
  obtenerClientesPos() {
    this.config.SubjectdataClientePos.subscribe((resp) => {
      this.clientes = resp;
      this.filtroClientes = new BehaviorSubject<ModelTerceroCompleto[]>(
        this.clientes
      );
    });
  }
  ...
  filtraTerceros(busqueda: string) {
    let filtro: ModelTerceroCompleto[] =
      this.metodos.filtrarArray<ModelTerceroCompleto>(
        this.clientes,
        'nombreComercial',
        busqueda
      );
  ...
  filtrarProductos(busqueda: string) {
    let filtro: Producto[] = this.metodos.filtrarArray<Producto>(
      this.productos,
      this.columnFiltroProducto,
      busqueda
    );
  ...
  seleccionarTercero(data: ModelTerceroCompleto) {
    ...
    this.formCotizacion.get('formaPago').setValue(data.formaPago.id);
    this.formCotizacion.get('vendedor').setValue(data.vendedor.id);
  }
  ...
  obtenerListadoPrecios() {
    ...
    if (this.producto.regulado) {
      let p: precios = {
        valor: this.metodos.redondearAl50MasCercano(
          this.producto.valorRegulacion
        ),
      };
      ...
      if (this.clienteSeleccionado.listaPrecios) {
        ...
        let p1: precios = {
          valor: this.metodos.redondearAl50MasCercano(
            this.producto.valorCompra / lista.precio1
          ),
        };
        let p2: precios = {
          valor: this.metodos.redondearAl50MasCercano(
            this.producto.valorCompra / lista.precio2
          ),
        };
        let p3: precios = {
          valor: this.metodos.redondearAl50MasCercano(
            this.producto.valorCompra / lista.precio3
          ),
        };
        ...
        let p1: precios = {
          valor: this.metodos.redondearAl50MasCercano(
            this.producto.valorCompra / 0.7
          ),
        };
        let p2: precios = {
          valor: this.metodos.redondearAl50MasCercano(
            this.producto.valorCompra / 0.75
          ),
        };
        let p3: precios = {
          valor: this.metodos.redondearAl50MasCercano(
            this.producto.valorCompra / 0.8
          ),
        };
        this.listaPrecios.push(p1);
        this.listaPrecios.push(p2);
  ...
  obtenerExistencias(data: InventarioProducto) {
    ...
    let cantidadUsada = 0;
    if (
      this.listadoDetalleFactura.filter(
        (d) => d.producto.id === data.idProducto.id && d.lote === data.lote
      )[0]
    ) {
  ...
  seleccionarInventario(data: InventarioProducto) {
    ...
    if (
      this.listadoDetalleFactura.filter(
        (d) => d.producto.id === this.producto.id && d.lote === this.lote
      )[0]
    ) {
      let detalle = this.listadoDetalleFactura.filter(
        (d) => d.producto.id === this.producto.id && d.lote === this.lote
  ...
  changeNumeracion(data: ModelNumeraciones) {
    if (data.tipoDocumento == '2') {
      ...
      this.formCotizacion.get('valorDomicilio').setValue(0);
    } else {
      this.domicilio = false;
  ...
  openModalProductos(content) {
    this.modalProducto = this.modalService.open(content, {
      size: 'xl',
      centered: true,
    });
  }
  ...
  setDetalle() {
    ...
    if (this.clienteSeleccionado.listaPrecios) {
      precioMinimo =
        this.valorCompra / this.clienteSeleccionado.listaPrecios.precioMinimo;
    }
  ...
  calcular() {
    ...
    if (this.producto.impuesto) {
      this.iva =
        ((this.valor - descuento) * this.producto.impuesto.porcentaje) / 100;
  ...
  calcularTotales(detalle: DetalleFactura[]) {
    ...
    this.formCotizacion.get('subtotal').setValue(this.subtotalFactura);
    this.formCotizacion.get('valorRetenciones').setValue(this.retencionFactura);
    this.formCotizacion.get('valorIva').setValue(this.ivaFactura);
    this.formCotizacion.get('descuento').setValue(this.descuentoFactura);
    this.formCotizacion.get('valor').setValue(this.totalFactura);
  }
  ...
  cancelarGuardado() {
    Swal.fire({
      icon: 'warning',
      title: 'Sarp Soft',
      text: 'esta seguro de cancelarla cotizacion?',
  ...
  guardarCotizacion() {
    this.mostrarOpcionesDomicilio().then(() => {
      Swal.fire({
        icon: 'info',
        title: 'Sarp Soft',
        text: 'esta seguro de guardar La cotizacion?',
        showCancelButton: true,
        confirmButtonText: 'SI',
      }).then((result) => {
        if (result.isConfirmed) {
          ...
          // const necesitaPermiso = this.listadoDetalleFactura.some(
          ...
          //   new MetodosShared()
          //     .AlertQuestion(
          //       'Lo sentimos, pero para acceder a los precios bajos y poder facturar, se requieren permisos adicionales. ¿Desea solicitar el permiso?'
          //     )
          //     .then((result) => {
          //       if (result.isConfirmed) {
          ...
          //           factura: this.formCotizacion.value,
          //           detalle: this.listadoDetalleFactura,
          //           usuario: {
          //             avatar: this.auth.currentUser.getAvatar(),
          //             nombre: this.auth.currentUser.getNombreCorto(),
          //             username: this.auth.currentUser.getUsername(),
          //           },
          //           autorizado: '',
          //         };

          //         this.fire_db
          //           .createDoc(data, 'autorizacionFacturas/', id)
          //           .then(() => {
          //             new MetodosShared().permisoPreciosBajos(
          //               this.generarMensajePermisosHTML(
          //                 this.listadoDetalleFactura,
          //                 id
          //               ),
          //               this.generarMensajePermisos(
          //                 this.listadoDetalleFactura,
          //                 id,
          //                 this.auth.currentUser.getUsername(),
          ...
          //             this.fire_db.crearNotificacionPush(
          //               msj,
          ...
          this.invoceServiceCotizacion
            .saveCotizacion(this.formCotizacion, this.listadoDetalleFactura)
            .subscribe(
              (resp: any) => {
                // console.log(resp);
                Swal.close();
                ...
                Swal.fire({
                  icon: 'success',
                  title: 'Sarp Soft',
                  text: `Cotizacion registrada con exito`,
                });
                this.obtenerNumeraciones();
                this.resetformFactura();
                this.resetTable();
  ...
  generarMensajePermisos(
    ...
    detalleFacturas.forEach((detalle) => {
      if (detalle.estado === false) {
        ...
        mensaje += `  Valor de Compra: *${currency.transform(
          detalle.valorCompra
        )}*\n`;
        mensaje += `  Valor de Venta: *${currency.transform(
          detalle.valor
        )}*\n\n`;
      }
    });
  ...
  generarMensajePermisosHTML(
    ...
    let currency = new CurrencyPipe('en-US');
    mensaje +=
      '<div class="alert alert-info" role="alert">Se requiere permiso para poder facturar a precios bajos los siguientes productos:</div>';
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (textoIngresado == undefined || textoIngresado == '') {
          ...
          let collectionSubscription: Subscription = this.fire_db
            .getCollectionQueryVariosParametros<any>(
            ...
            .subscribe((resp) => {
              if (resp.length > 0) {
                ...
                this.invoceService
                  .saveFacturaPermitida(obj.factura, obj.detalle)
                  .subscribe(
                    (resp: any) => {
                      Swal.close();
                ...
                new MetodosShared().AlertError(
                  'Lo sentimos, no se encontró ningún documento con el código proporcionado o aún no ha sido aprobado.'
                );
              }
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        const values = Array.from(
          document.getElementsByClassName('swal-word')
