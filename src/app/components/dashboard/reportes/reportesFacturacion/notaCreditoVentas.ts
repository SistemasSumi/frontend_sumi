...
export class NotaCreditoVentasReport {
    private cp:CurrencyPipe = new CurrencyPipe('en-US');
    
    constructor(){}

    public reporteNotaCreditoVentas(data?:any,conta?:ModelAsiento){
        ...
        autoTable(doc, {
          ...
          didDrawPage: ({ pageNumber, doc: jsPDF }) => {
            ...
            if(data.isElectronica){
              ...
              doc.text('NOTA CRÉDITO', 307, 125, { align: "center" })
            }else{

              doc.text('NOTA CRÉDITO POS', 307, 125, { align: "center" })
            }
            doc.setFont(undefined, 'normal')
            doc.setFontSize(9)
            ...
            doc.text(acortarText.transform(data.cliente.nombreComercial,"80").toUpperCase(),86, startYText);
            ...
            doc.setTextColor("#000");
            doc.setFont(undefined, 'normal')
            doc.text(data.cliente.documento,384.25, startYText);
            ...
            doc.setTextColor("#000");
            doc.setFont(undefined, 'normal')
            doc.text(data.cliente.telefonoContacto || '',508.75, startYText);
            ...
            doc.setFont(undefined, 'normal')
            doc.text(acortarText.transform(data.cliente.direccion,"46").toUpperCase(),86, 174);
            ...
            doc.setTextColor("#FFF");
            doc.setFont(undefined, 'bold')
            doc.text("FACTURA N°:",17, 193);
            doc.setTextColor("#000");
            doc.setFont(undefined, 'normal')
            doc.text(data.cxc.numero,86, 193);

            doc.setFillColor("#41B6FF");
            doc.rect(152.25,181,50,18,'FD');
            doc.rect(202.25,181,205.75,18);
            doc.setTextColor("#FFF");
            doc.setFont(undefined, 'bold')
            doc.text("CORREO:",154.25, 193);
            doc.setTextColor("#000");
            doc.setFont(undefined, 'normal')
            doc.text(data.cliente.correoFacturas,204.25, 193);
            ...
            doc.setTextColor("#FFF");
            doc.setFont(undefined, 'bold')
            doc.text("FECHA DE VENTA:",410, 193);
            doc.setTextColor("#000");
            doc.setFont(undefined, 'normal')
            doc.text(moment(data.cxc.fecha).format("DD/MM/YYYY").toUpperCase(),508.75, 193); 



            doc.setFillColor("#41B6FF");
            doc.rect(15,199,80,18,'FD');
            doc.rect(95,199,500.5,18);
            doc.setTextColor("#FFF");
            doc.setFont(undefined, 'bold')
            doc.text("Observaciones:",17.25, 211);
            doc.setTextColor("#000");
            doc.setFont(undefined, 'normal')
            doc.text(data.observacion,97.25, 211);
        ...
        if(data.contabilizado){
          ...
          finalText+=15
          doc.line(15,finalText,590,finalText);

          finalText+=10
          doc.text(this.cp.transform(totalDebito),476, finalText,"right");
          doc.text(this.cp.transform(totalCredito),590, finalText,"right");
